...
class MyCalander extends JFrame {
	...
	private int month, year, day; //to store the year, month and day of the week
	private	boolean leap; //to store if it is a leap year or not
	...
	private JTextField t1= new JTextField("Enter a  Year"); //to get the year
	private JButton b1=new JButton("Generate Calander");  // button to make the calander
	...
	//to initialize the interface of the calander
	public void makeGui() {
		setTitle("Java Calander");
		c=getContentPane();	
		...
		b3.setEnabled(false);
		b4.setEnabled(false);
		Icon ico=new ImageIcon("compose.png");  //icon in calander butto, check for currect path
		b1.setIcon(ico); 
	...
	// to clear all the lables and make the >>,<< buttons as dissabled
	public void clear() {
		for(int x=0; x<42; x++) {
			lbldays[x].setText("");
		}
